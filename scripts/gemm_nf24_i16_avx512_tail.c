...
int triposplat_gemm_rnf8_avx512_tail(
  ...
  if (M <= 0 || K <= 0 || N <= 0 || stride_x < K || stride_out < N || stages != 3) return -2;
#ifdef _OPENMP
  ...
  for (int i = 0; i < M; ++i) {
    ...
    for (int n = 0; n < N; ++n) {
      ...
      for (int k = 0; k < K; ++k) {
        ...
        const int32_t q = (int32_t)((const int16_t*)codes0_t)[offset] * 256
                        + (int32_t)(int8_t)codes1_t[offset];
        const float weight = (float)q * scales0[n];
