...
STEPS="${STEPS:-1}"
RUN_ID="${RUN_ID:-cpu_avx512_nf8_strict_s${STEPS}_g3_1024}"
...
required_libraries=(
  libtriposplat_gemm_nf8_avx512.so
...
env \
  ...
  "${VENV_PY}" scripts/run_triposplat_nf8_param_batch.py \
    ...
    --native-avx512-linear-library artifacts/backends/libtriposplat_gemm_nf8_avx512.so \
