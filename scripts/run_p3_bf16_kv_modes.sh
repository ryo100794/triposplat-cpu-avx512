...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/artifacts/quant_flow/p3_bf16_kv_modes_s1}"
AUDIT_ROOT="${AUDIT_ROOT:-${PROJECT_ROOT}/artifacts/audits/p3_bf16_kv_modes_s1}"
MODES="${MODES:-k,v,kv}"
...
for path in "${VENV_PY}" "${TRIPOSPLAT_REPO}/triposplat.py" "${INPUT}" "${CONDITION_NPZ}" "${NOISE_NPZ}" "${REFERENCE_NPZ}" "${PACKED_SDPA_LIBRARY}" "${NATIVE_SDPA_LIBRARY}" "${RNF8_LIBRARY}"; do
...
for mode in "${requested_modes[@]}"; do
  case "${mode}" in k|v|kv) ;; *) printf 'Unknown mode: %s\n' "${mode}" >&2; exit 2 ;; esac
  qkv_library="${PROJECT_ROOT}/artifacts/backends/libtriposplat_qkv_postprocess_bf16_${mode}_probe_gcc13_znver4.so"
  [[ -f "${qkv_library}" ]] || { printf 'Probe library is missing: %s\n' "${qkv_library}" >&2; exit 2; }
  output_dir="${OUTPUT_ROOT}/${mode}"
  compare_json="${AUDIT_ROOT}/${mode}_vs_exact.json"
  mkdir -p "${output_dir}"
  ...
  env \
    ...
    TRIPOSPLAT_QKV_POSTPROCESS_LIBRARY="${qkv_library}" \
    ...
    "${VENV_PY}" scripts/run_triposplat_rnf8_packed_v3_param_batch.py \
      ...
      --steps 1 --guidance-scale 3 --shift 3 --canvas-size 1024 --seed 0 \
  ...
  "${VENV_PY}" -c 'import json,sys; p=json.load(open(sys.argv[1])); print(json.dumps({"mode":sys.argv[2],"combined_rmse":p["combined_rmse_from_key_mse"],"latent_rmse":p["per_key"]["latent"]["rmse"],"camera_rmse":p["per_key"]["camera"]["rmse"]}))' "${compare_json}" "${mode}"
done
