...
CC="${CC:-gcc}"
OUT="${OUT:-$ROOT/artifacts/backends/libtriposplat_gemm_nf24_qkv_direct_tile_avx512.so}"
...
"$CC" "${CFLAGS_ARRAY[@]}" "${ARCH_FLAGS_ARRAY[@]}" "${OPENMP_FLAGS_ARRAY[@]}" -shared \
  "$ROOT/scripts/gemm_nf24_qkv_direct_tile_avx512.c" "${LDFLAGS_ARRAY[@]}" -lm -o "$OUT"
ls -lh "$OUT"
