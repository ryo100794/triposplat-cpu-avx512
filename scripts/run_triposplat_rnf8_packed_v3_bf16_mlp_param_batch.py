...
from triposplat_bf16_roundtrip_patch import apply_triposplat_bf16_output_roundtrip_patch
...
def install_packed_helpers_and_bf16_probe() -> None:
    ...
    def apply_with_helpers(flow_model, **kwargs):
        ...
        bf16 = apply_triposplat_bf16_output_roundtrip_patch(
            flow_model,
            enabled=os.environ.get("TRIPOSPLAT_BF16_MLP_PROBE", "1") == "1",
            include_regex=os.environ.get("TRIPOSPLAT_BF16_MLP_INCLUDE_REGEX", r"^blocks[.][0-9]+[.]mlp[.]mlp[.]1$"),
            exclude_regex=os.environ.get("TRIPOSPLAT_BF16_MLP_EXCLUDE_REGEX"),
        )
        return {"base": base, "packed_qkv_helpers": packed, "bf16_output_probe": bf16}

    triposplat_attention_patch.apply_triposplat_module_attention_patch = apply_with_helpers


install_prepacked_loader()
install_packed_helpers_and_bf16_probe()
