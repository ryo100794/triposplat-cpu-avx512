...
CC="${CC:-gcc}"
OUT="${OUT:-$ROOT/artifacts/backends/libtriposplat_sdpa_avx512_exact_q8t512_packed.so}"
...
"$CC" "${CFLAGS_ARRAY[@]}" "${ARCH_FLAGS_ARRAY[@]}" "${OPENMP_FLAGS_ARRAY[@]}" -shared \
  "$ROOT/scripts/native_sdpa_avx512_exact_q8t512_packed.c" "${LDFLAGS_ARRAY[@]}" -lm -o "$OUT"
ls -lh "$OUT"
