...
| NF24 int16、先頭2段統合（24 bit） | 75.11% | s20 combined RMSE 9.37666e-5 | 採用、2747.270秒 |

採用したNF24 int16 row tile 8 + exact SDPA query 8/key tile 512のs20は、
project-local GCC13 Zen 4 buildで2747.270秒です。旧NF24 3471.330秒から724.060秒
（20.9%）、strict float32 AVX-512から575.616秒、元のCPU float32から8109.118秒
短縮しました。native packed Linearは1008.472秒、SDPAは1432.319秒、camera RMSEは
8.93055e-6で、NaN、Inf、fallbackはすべて0です。TripoSplatの意味を変えず3000秒
未満のgateを達成しました。
...
に整理しました。初期refの局所処理、streaming、早期skipという着眼をFlowへ対応付け、
演算融合、再利用、許容誤差付き近似、inline assemblyへ進む条件を分離しています。
P0/P1の実測と採否理由は
